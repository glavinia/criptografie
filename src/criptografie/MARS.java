...
public class MARS {
	...
	private static int[] expandeazaCheie(byte[] key){
		int n = key.length/4;
		int[] tmp = new int[40];
		int[] data = new int[n];
	...
	private static int genereazaMasca(int x){
		int m;
	...
	public static byte[] blocCriptare(byte[] in){
	...
	private static int[] fct_E(int in,int k1,int k2){
		int[] tmp = new int[3];
		int M,L,R;
		M = in + k1;
		R = rotl(in,13) * k2;
	...
	public static byte[] blocDecriptare(byte[] in){
		byte[] tmp =  new byte[in.length];
		int aux;
	...
	public static byte[] cripteaza(byte[] in,byte[] key){
		K = expandeazaCheie(key);
		int lenght=0;
	...
	public static byte[] decriptare(byte[] in,byte[] key){
		byte[] tmp = new byte[in.length];
		byte[] bloc = new byte[16];
		K = expandeazaCheie(key);
		int i;
