...
public class AES {
	...
	private static byte[] fct_xor(byte[] a, byte[] b) {
	...
	private static byte[][] genereazaSubChei(byte[] key) {
		...
		while (i < Nb * (Nr + 1)) {
			...
			for(int k = 0;k<4;k++)
				temp[k] = tmp[i-1][k];
			if (i % Nk == 0) {
				temp = SubCuvant(roteazaCuvant(temp));
				temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
			} else if (Nk > 6 && i % Nk == 4) {
				temp = SubCuvant(temp);
			}
			tmp[i] = fct_xor(tmp[i - Nk], temp);
			i++;
		}
	...
	private static byte[] SubCuvant(byte[] in) {
	...
	private static byte[] roteazaCuvant(byte[] input) {
	...
	private static byte[][] AdaugaCheieI(byte[][] state, byte[][] w, int round) {
	...
	private static byte[][] SubBiti(byte[][] state) {
	...
	private static byte[][] InverseazaSubBiti(byte[][] state) {
		for (int row = 0; row < 4; row++) 
			for (int col = 0; col < Nb; col++)
	...
	private static byte[][] SifteazaRand(byte[][] state) {
	...
	private static byte[][] InvSifteazaRand(byte[][] state) { 
		byte[] t = new byte[4]; 
		for (int r = 1; r < 4; r++) {
			for (int c = 0; c < Nb; c++) 
				t[(c + r)%Nb] = state[r][c];
	...
	public static byte[] blocCriptare(byte[] in) {
		...
		state = AdaugaCheieI(state, w, 0);
		for (int round = 1; round < Nr; round++) {
			state = SubBiti(state);
			state = SifteazaRand(state);
			state = MixColumns(state);
			state = AdaugaCheieI(state, w, round);
		}
		state = SubBiti(state);
		state = SifteazaRand(state);
		state = AdaugaCheieI(state, w, Nr);
	...
	public static byte[] blocDecriptare(byte[] in) {
		...
		for (int round = Nr-1; round >=1; round--) {
			state = InverseazaSubBiti(state);
			state = InvSifteazaRand(state);
			state = AdaugaCheieI(state, w, round);
	...
	public static byte[] decripteaza(byte[] in,byte[] key){
		int i;
		byte[] tmp = new byte[in.length];
		byte[] bloc = new byte[16];
